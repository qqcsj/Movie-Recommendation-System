...
class Login(forms.Form):
    username = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'Username',  # 这里设置显示的文字：Name
            'style': 'background-color: transparent; color: white; border: none; border-bottom: 1px solid grey;'
            # style解释: 背景透明，文字白色，去边框，只留底线
        })
    )
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(attrs={
            'class': 'form-control',
            'placeholder': 'Password',  # 这里设置显示的文字：Password
            'style': 'background-color: transparent; color: white; border: none; border-bottom: 1px solid grey;'
        })
    )
